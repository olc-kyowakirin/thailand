...
$(function() {
    ...
    //noscript対応
    $('#index1').css('float', 'left');
    $('#index2,#index3,#index #latestNews ul').show();
    ...
    if ($('#index').size() > 0) {
        ...
        for (var i = 0; i < $config['main'].length; i++) {
            ...
            if (c['link']) {
                ...
                m.live('click', function() {
                    location.href = $config['main'][$(this).attr('id').replace(/index/, '') - 1]['link'];
                });
            }
        }
        ...
        $('#carousel').carouFredSel({
            auto: {
                pauseDuration: 5000,
                onAfter: function(before, after) {
                    $('#button li').eq(parseInt($(before).attr('id').replace(/^index/, '')) - 1).removeClass('current');
                    $('#button li').eq(parseInt($(after).attr('id').replace(/^index/, '')) - 1).addClass('current');
                }
            },
            scroll: {
                duration: 1000
            ...
            pagination: {
                ...
                anchorBuilder: function(nr, item) {
                    return '<li' + (nr === 1 ? ' class="current"' : '') + '><a href="javascript:void(0);"><img src="/thailand/share/images/pct_thumbnail0' + nr + '.jpg" width="41" height="26" /></a></li>';
                },
                onAfter: function(before, after) {
                    $('#button li').eq(parseInt($(before).attr('id').replace(/^index/, '')) - 1).removeClass('current');
                    $('#button li').eq(parseInt($(after).attr('id').replace(/^index/, '')) - 1).addClass('current');
        ...
        $('#index ul#button li').click(function() {
        ...
        var $l = $('.latest_news_body'),
            cn = 0,
            total_news = 3;
        //$l.css('opacity',0);	//**hide ->透明度に変更
        //$l.hide();
        for (var i = 1; i <= total_news; i++) {
            var f = $config['news' + (i)];
            if (f['text']) {
                replaceNews(i);
                //break;
        ...
        /*
        		//初回生成
        		$('span.txtLength').html('ABCDEFG');
        		var oh = $('span.text').height()*2-1;		//**2行高さ初期値取得追加

        		var $l=$('.latest_news_body'), cn=0, total_news=3;
        		//$l.hide();
        		 for ( var i=0; i<total_news; i++ ) {
        			var f=$config['news'+(i+1)];
        			if ( f['text'] ) {
        				replaceNews(i);
        				break;
        			}
        		...
        		//timerセット
        		var standard_flow = function() {
        			cn++;
        			replaceNews(total_news < (cn+1) ? 0 : cn);
        		};
        		var timer = setInterval(standard_flow, 7000);

        		//左スクロール
        		$('#latestNews ul li').eq(0).click(function(){
        			cn--;
        			replaceNews(0 > cn ? total_news-1 : cn);
        			clearInterval(timer);
        		});

        		//右スクロール
        		$('#latestNews ul li').eq(1).click(function(){
        			cn++;
        			replaceNews(total_news-1 < cn ? 0 : cn);
        			clearInterval(timer);
        		});
        */
    }
    ...
    function replaceNews(i) {
        ...
        if (f['text']) {
            ...
            //置換
            if (f['date']) {
                $('#latest_date' + i, $l).text(f['date']);
            }
            if (f['link']) {

                if (f['text'].length > 172) { // *100 -> 172に変更
                    f['text'] = f['text'].slice(0, 172) + '...';
                }
                $('#latest_body' + i, $l).html('<a href="' + f['link'] + '">' + $('<div/>').text(f['text']).html() + '</a>'); //**タグ構造変更

            } else if (f['pdf']) {
                if (f['text'].length > 148) { // *88 -> 148に変更
                    f['text'] = f['text'].slice(0, 148) + '...';
                }
                $('#latest_body' + i, $l).html('<a href="' + f['pdf'] + '" target="_blank"><span class="label pdf">PDF</span>' + (f['pdf_capa'] ? '(' + f['pdf_capa'] + ')' : '') + '' + $('<div/>').text(f['text']).html() + '</a>'); //**タグ構造変更

            } else {
                if (f['text'].length > 172) {
                    f['text'] = f['text'].slice(0, 172) + '...';
                }
                $('#latest_body' + i, $l).html($('<div/>').text(f['text']).html()); //**タグ構造変更
            }
...
(function(a, b, c) {
    ...
    s.special.smartresize = {
        ...
        handler: function(a, b) {
            var c = this,
                d = arguments;
            a.type = "smartresize", t && clearTimeout(t), t = setTimeout(function() { jQuery.event.handle.apply(c, d) }, b === "execAsap" ? 0 : 100)
        }
    ...
    b.Isotope.settings = { resizable: !0, layoutMode: "masonry", containerClass: "isotope", itemClass: "isotope-item", hiddenClass: "isotope-hidden", hiddenStyle: { opacity: 0, scale: .001 }, visibleStyle: { opacity: 1, scale: 1 }, containerStyle: { position: "relative", overflow: "hidden" }, animationEngine: "best-available", animationOptions: { queue: !1, duration: 800 }, sortBy: "original-order", sortAscending: !0, resizesContainer: !0, transformsEnabled: !b.browser.opera, itemPositionDataEnabled: !1 }, b.Isotope.prototype = {
        _create: function(a) {
            ...
            var i = this;
            setTimeout(function() { i.element.addClass(i.options.containerClass) }, 0), this.options.resizable && v.bind("smartresize.isotope", function() { i.resize() }), this.element.delegate("." + this.options.hiddenClass, "click", function() { return !1 })
        },
    ...
    var w = function(b) { a.console && a.console.error(b) };
    b.fn.isotope = function(a, c) {
        if (typeof a == "string") {
            var d = Array.prototype.slice.call(arguments, 1);
            this.each(function() {
                var c = b.data(this, "isotope");
                if (!c) { w("cannot call methods on isotope prior to initialization; attempted to call method '" + a + "'"); return }
                if (!b.isFunction(c[a]) || a.charAt(0) === "_") { w("no such method '" + a + "' for isotope instance"); return }
                c[a].apply(c, d)
            })
...
$(function() {
    ...
    if ($('#over_view').size() > 0) {
        ...
        var $container1 = $('#over_view #container'),
            $container2 = $('#recent_changes #container'),
            current_key = '';

        //タブの切り替え
        $('#pipelineMenu li').click(function() {
            switch ($(this).attr('id')) {
                case 'recent':
                    $('#over_view').hide();
                    $('#recent_changes,#dev_date').show();
                    break;
                default:
        ...
        //フィルタ初期チェック
        $('table.filter input').attr('checked', false);
        $('table.filter input[name*=_all]').attr('checked', true);
        ...
        $container1.isotope({
            ...
            getSortData: {
                compound: function($elem) {
                    var tmp = $elem.find('.compound').html();
                    if (tmp == '&nbsp;' || tmp == '-&nbsp;') {
                        tmp = 'ZZZZZ&nbsp;';
                    }
                    tmp += $elem.find('.compound').siblings('span').text();
                    return tmp;
                },
                phase: function($elem) {
                    var tmp = $elem.find('.head04 p').attr('class').split(/p/);
                    return ((isIE6 || isIE7) ? tmp : tmp[1]);
                }
            }
        });
        ...
        $('#options .option-set a').click(function() {
            ...
            if (value == 'phase') {
                ...
                options['sortAscending'] = $(this).hasClass('asc') ? true : false;
                ...
                options['sortAscending'] = $(this).hasClass('asc') ? false : true;
        ...
        //Code Nameクリック時
        $('#over_view #container .element a.compound').click(function() {
            var $a = $(this).parents().parents().siblings('.open');
            if ($a.is(':hidden')) {
                $a.fadeIn(700);
                $container1.isotope('reLayout');
                $a.parents('.element').addClass('current');
            } else {
                $a.fadeOut('fast', function() {
                    $container1.isotope('reLayout');
                });
                $a.parents('.element').removeClass('current');
            }
            return false;
        });

        //レコードクリック時
        $('#over_view #container div.element').click(function() {
            var $a = $('.open', this);
            if ($a.is(':hidden')) {
                $a.fadeIn(700);
                $container1.isotope('reLayout');
        ...
        $('table.filter input').click(function() {
            ...
            var a = $(this).attr('name'),
                fil_opt = [],
                operator = '',
                reverse = { 'modalities': 'therapeuticarea', 'therapeuticarea': 'modalities' };
            ...
            switch (a) {
                ...
                case 'therapeuticarea_all':
                    ...
                    $('input[name=' + reverse[b] + ']:checked').each(function() {
                        fil_opt.push('.' + $(this).val());
                    });
                    operator = ',';
                    break;
                ...
                default:
                    ...
                    if ($c.length === 0) {
                        ...
                        $('input[name=' + a + '_all]').attr('checked', true);

                        $('input[name=' + reverse[a] + ']:checked').each(function() {
                            fil_opt.push('.' + $(this).val());
                        });
                        operator = ',';
                        ...
                        $('input[name=' + a + '_all]').attr('checked', false);

                        //to filter for (x OR y) AND (a OR b), the selector would look like '.x.a, .x.b, .y.a, .y.b'.
                        $('input[name=' + a + ']:checked').each(function() {
                            var tmp = '.' + $(this).val();
                            if ($('input[name=' + reverse[a] + ']:checked').length > 0) {
                                $('input[name=' + reverse[a] + ']:checked').each(function() {
                                    fil_opt.push(tmp + '.' + $(this).val());
                                });
                            } else {
                                fil_opt.push(tmp);
                            }
            ...
            if (fil_opt.length == 0) {
                ...
                total_opt = fil_opt.join(operator);
            }

            $container1.isotope({ filter: total_opt });
        });

        //RecentChanges Isotope生成
        $container2.isotope({
            itemSelector: '.element',
            layoutMode: 'straightDown',
            animationEngine: 'best-available'
        ...
        //Code Nameクリック時
        $('#recent_changes #container .element a.compound').click(function() {
            var $a = $(this).parents().parents().siblings('.open');
            if ($a.is(':hidden')) {
                $a.fadeIn(700);
        ...
        //レコードクリック時
        $('#recent_changes #container div.element').click(function() {
            var $a = $('.open', this);
            if ($a.is(':hidden')) {
                $a.fadeIn(700);
                $container2.isotope('reLayout');
        ...
        //レコードhover時(for IE6)
        if (isIE6) {
            $('#over_view #container div.element, #recent_changes #container div.element').hover(
                function() {
                    $(this).css('background', '#e7e5e3 url(/thailand/share/images/bg_overview_data01.gif) repeat-x');
                },
                function() {
                    $(this).css('background', '');
                }
        ...
        //NewsReleasesでのみ実行
    } else if ($('#news_releases_isotope').size() > 0) {

        var $container = $('#news_releases_isotope #container');

        //NewsReleases Isotope生成
        $container.isotope({
            itemSelector: '.element',
            layoutMode: 'straightDown',
            animationEngine: 'best-available'
        ...
        //Searchボタンクリック時
        $('#searchArea #btnSearch').click(function() {
            ...
            //検索条件収集
            $('#searchArea ul.year input:checked').each(function() {
                cond2 += '.' + $(this).attr('id');
            });
            $('#searchArea ul.category input:checked').each(function() {
                cond1.push(cond2 + '.' + $(this).attr('id'));
            });
            ...
            if (cond1.length === 0 && cond2.length === 0) {
                ...
                total_opt = cond1.join(operator);
            }

            $container.isotope({ filter: total_opt });

            //カウント0件の場合
            var $filtered = $('#container').data('isotope').$filteredAtoms;
            if ($filtered.filter('.element').length == 0) {
                $('#error').show();
            } else {
                $('#error').hide();
            }
...
(function(f, e) {
    ...
    function u(h, b, o, j, p) {
        ...
        if (c.random) {
            c.randomMap = [];
            for (k = 0; k < o.length; k++) c.randomMap.push(k);
            c.randomMap.sort(function() { return Math.random() - 0.5 });
            if (i)
                for (i = 0; i < o.length; i++) c.startingSlide == c.randomMap[i] && (c.randomIndex = i);
            else c.randomIndex = 1, c.startingSlide = c.randomMap[1]
        } else c.startingSlide >= o.length && (c.startingSlide = 0);
        ...
        d(o[t], c);
        c.fit && (c.aspect ? b.each(function() {
            var b = f(this),
                a = c.aspect === true ? b.width() / b.height() : c.aspect;
        ...
        }) : (c.width && b.width(c.width), c.height && "auto" != c.height && b.height(c.height)));
        ...
        c.center && (!c.fit && !c.slideResize) && b.each(function() {
            var b = f(this);
        ...
        var C = !1;
        c.pause && h.bind("mouseenter.cycle", function() {
            C = true;
            this.cyclePause++;
            l(u, true)
        ...
        b.each(function() {
            ...
            if (b.is("img")) {
                var b = f.browser.mozilla && this.cycleW == 34 && this.cycleH == 19 && !this.complete,
                    ...
                    0 && !this.complete;
                if (f.browser.msie && this.cycleW == 28 && this.cycleH == 30 && !this.complete || b || h || d) {
                    if (p.s && c.requeueOnImageNotLoaded && ++j.requeueAttempts < 100) {
        ...
        if (c.timeout) {
            c.timeout = parseInt(c.timeout, 10);
            c.speed.constructor == String && (c.speed = f.fx.speeds[c.speed] || parseInt(c.speed, 10));
            c.sync || (c.speed /= 2);
            for (q = "none" == c.fx ? 0 : "shuffle" == c.fx ? 500 : 250; c.timeout - c.speed < q;) c.timeout += c.speed
        }
        ...
        if (c.pager || c.pagerAnchorBuilder) {
            var x = c,
                E = f(x.pager);
        ...
        m.addSlide = function(b, a) {
            ...
            if (a) {
                m.currSlide++;
                m.nextSlide++
            }!f.support.opacity && (m.cleartype && !m.cleartypeNoBg) && B(c);
    ...
    function v(a, b) {
        ...
        else if (a.nextSlide = a.currSlide + d, 0 > a.nextSlide) {
    ...
    f.fn.cycle = function(d, b) {
        ...
        if (this.length === 0 && d != "stop") {
            if (!f.isReady && e.s) {
                a("DOM not ready, queuing slideshow");
                f(function() { f(e.s, e.c).cycle(d, b) });
                return this
            }
    ...
    f.fn.cycle.resetState = function(a, b) {
        ...
        f.each(a.original.after, function() { a.after.push(this) });
        var d = f.fn.cycle.transitions[b];
    ...
    f.fn.cycle.hopsFromLast = function(a, b) {
        var f = a.lastSlide,
            d = a.currSlide;
        return b ? d > f ? d - f : a.slideCount - f : d < f ? f - d : f + a.slideCount - d
    };
    ...
    f.fn.cycle.transitions = {
        fade: function(a, b, d) {
            b.not(":eq(" + d.currSlide + ")").css("opacity", 0);
            d.before.push(function(a, b, d) {
                f.fn.cycle.commonReset(a, b, d);
...
(function(f) {
    ...
    f.fn.cycle.transitions.fadeout = function(e, g, a) {
        ...
        a.before.push(function(a, e, d, g, r, n) {
            f(a).css("zIndex", d.slideCount + (!0 !== n ? 1 : 0));
    ...
    f.fn.cycle.transitions.shuffle = function(e, g, a) {
        ...
        a.fxFn = function(a, e, d, g, r) {
            ...
            n.animate(d.shuffle, d.speedIn, d.easeIn, function() {
                ...
                if (r) { e = 0; for (j = d.els.length; e < j; e++) f(d.els[e]).css("z-index", j - e + v) } else {
                    e = f(a).css("z-index");
                    n.css("z-index", parseInt(e, 10) + 1 + v)
                }
                n.animate({ left: 0, top: 0 }, d.speedOut, d.easeOut, function() {
                    f(r ? this : a).hide();
                    g && g()
                })
            })
    ...
    f.fn.cycle.transitions.fadeZoom = function(e, g, a) {
        a.before.push(function(a, e, d) {
            f.fn.cycle.commonReset(a, e, d, !1, !1);
            d.cssBefore.left = e.cycleW / 2;
            d.cssBefore.top = e.cycleH / 2;
            f.extend(d.animIn, { top: 0, left: 0, width: e.cycleW, height: e.cycleH })
        });
    ...
    f.fn.cycle.transitions.curtainX = function(e, g, a) {
        a.before.push(function(a, e, d) {
            f.fn.cycle.commonReset(a, e, d, !1, !0, !0);
            d.cssBefore.left = e.cycleW / 2;
            d.animIn.left = 0;
            d.animIn.width = this.cycleW;
    ...
    f.fn.cycle.transitions.curtainY = function(e, g, a) {
        a.before.push(function(a, e, d) {
            f.fn.cycle.commonReset(a, e, d, !0, !1, !0);
            d.cssBefore.top = e.cycleH / 2;
            d.animIn.top = 0;
            d.animIn.height = e.cycleH;
...
$(function() {
    ...
    //Investorsでのみ実行
    if ($('#investorContent').size() > 0) {

        var total = 0;
        for (var i = 0; i < $config['topics'].length; i++) {
            var c = $config['topics'][i],
                $u = $('#investorContent ul.list'),
                p = '';
            if (c['disp'] === '1') {
                p += '<li style="margin:0;with:500px">';
                if (c['img']) {
                    p += '<div class="textBox">';
                }
                p += '<p>';
                if (c['date']) {
                    p += '<span>' + $('<div/>').text(c['date']).html() + '</span><br />';
                }
                if (c['title1']) {
                    p += '<strong>' + $('<div/>').text(c['title1']).html() + '</strong>';
                }
                p += '</p>';
                if (c['title2']) {
                    p += '<p><strong>' + $('<div/>').text(c['title2']).html() + '</strong></p>';
                }
                if (c['caption']) {
                    p += '<p>' + $('<div/>').text(c['caption']).html() + '</p>';
                }
                p += '<ul>';
                if (c['pdftitle1']) {
                    var t = $('<div/>').text(c['pdftitle1']).html();
                    var capa = $('<div/>').text(c['pdf_capa1']).html();
                    p += '<li><img src="images/pct_pdf.gif" width="14" height="15" alt="' + t + '" />&nbsp;<a href="' + $('<div/>').text(c['pdf1']).html() + '" target="_blank">' + t + '</a>&nbsp;' + (capa ? '(' + capa + ')' : '') + '</li>';
                }
                if (c['pdftitle2']) {
                    var t = $('<div/>').text(c['pdftitle2']).html();
                    var capa = $('<div/>').text(c['pdf_capa2']).html();
                    p += '<li><img src="images/pct_pdf.gif" width="14" height="15" alt="' + t + '" />&nbsp;<a href="' + $('<div/>').text(c['pdf2']).html() + '" target="_blank">' + t + '</a>&nbsp;' + (capa ? '(' + capa + ')' : '') + '</li>';
                }
                if (c['pdftitle3']) {
                    var t = $('<div/>').text(c['pdftitle3']).html();
                    var capa = $('<div/>').text(c['pdf_capa3']).html();
                    p += '<li><img src="images/pct_pdf.gif" width="14" height="15" alt="' + t + '" />&nbsp;<a href="' + $('<div/>').text(c['pdf3']).html() + '" target="_blank">' + t + '</a>&nbsp;' + (capa ? '(' + capa + ')' : '') + '</li>';
                }
                if (c['soundtitle']) {
                    var t = $('<div/>').text(c['soundtitle']).html();
                    p += '<li><img src="images/pct_sound.gif" width="17" height="14" alt="' + t + '" />&nbsp;<a href="' + $('<div/>').text(c['sound']).html() + '" target="_blank">' + t + '</a></li>';
                }
                if (c['movietitle']) {
                    var t = $('<div/>').text(c['movietitle']).html();
                    p += '<li><img src="images/pct_movie.gif" width="16" height="11" alt="' + t + '" />&nbsp;<a href="' + $('<div/>').text(c['movie']).html() + '" target="_blank">' + t + '</a></li>';
                }
                if (c['linktitle_out']) {
                    var t = $('<div/>').text(c['linktitle_out']).html();
                    p += '<li><img src="images/pct_blank.gif" width="17" height="14" alt="' + t + '" />&nbsp;<a href="' + $('<div/>').text(c['link_out']).html() + '" target="_blank">' + t + '</a></li>';
                }
                if (c['linktitle_in']) {
                    var t = $('<div/>').text(c['linktitle_in']).html();
                    p += '<li><a href="' + $('<div/>').text(c['link_in']).html() + '">' + t + '</a></li>';
                }
                p += '</ul>';
                if (c['img']) {
                    p += '</div><div class="imageBox"><p><img src="' + $('<div/>').text(c['img']).html() + '" width="120" alt="image" /></p></div>';
                }
                p += '</li>';
                $u.html($u.html() + p);
                total++;
        ...
        if (total > 1) {
            $('#investorContent ul.list').cycle({
                speed: 200,
                timeout: 10000,
                pager: 'ul#slotFocus-pagination',
                pagerAnchorBuilder: slotTopics_buildPagination,
                prevNextClick: slotTopics_actionPause,
...
function slotTopicsPlayPause() {
    $('ul#slotFocus-pagination li:last a').click(function(e) {
        e.preventDefault();
        if ('play' == $(this).attr('class')) {
            $(this).attr('class', 'pause');
            $('#investorContent ul.list').cycle('resume');
            $('img', this).attr('src', $('img', this).attr('src').replace(/txt\_start/, 'txt_pause'));
            $('img', this).attr('alt', $('img', this).attr('alt').replace(/start/, 'pause'));
        } else {
            $(this).attr('class', 'play');
            $('#investorContent ul.list').cycle('pause');
            $('img', this).attr('src', $('img', this).attr('src').replace(/txt\_pause/, 'txt_start'));
            $('img', this).attr('alt', $('img', this).attr('alt').replace(/pause/, 'start'));
        }
    });
}

function slotTopics_buildPagination(idx, slide) {
    return '<li><a href="#"><img src="/investors/images/txt_tab' + (idx + 1) + (idx == 0 ? '_on' : '') + '.gif" width="56" height="24" alt="' + (idx + 1) + '" /></a></li>';
}

function slotTopics_actionPause(isNext, zeroBasedSlideIndex, slideElement) {
    $('#investorContent ul.list').cycle('pause');
    setTimeout(function() { $('#investorContent ul.list').cycle('resume'); }, 10000);
}

function onAfter() {
    var $a = $('ul#slotFocus-pagination li.selected img');
    $('ul#slotFocus-pagination li img').each(function() {
        $(this).attr('src', $(this).attr('src').replace(/\_on\.gif$/, '.gif'));
    });
    if ($a.attr('src')) {
        $a.attr('src', $a.attr('src').replace(/\.gif$/, '_on.gif'));
    }
}